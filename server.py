...
def get_quotes():
    ...
    if 'author' in request.query and 'ip' in request.query:
        return json.dumps(p.filter_by_author(request.query['author'], p.find_by_ip(request.query['ip'])))
    if 'author' in request.query:
        return json.dumps(p.filter_by_author(request.query['author'], p.all_quotes()))
